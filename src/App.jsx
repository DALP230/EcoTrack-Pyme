...
function App() {
    ...
    return (
      <div style={{ 
        ...
        <div style={{ textAlign: 'center', marginBottom: '-15px', marginTop: '-20px', width: '100%', maxWidth: '840px', zIndex: 1 }}>
          <img src={loginLogoUrl} alt="Logo Grande EcoTrack" style={{ height: '420px', width: 'auto', display: 'block', margin: '0 auto', objectFit: 'contain' }} />
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '25px' }}>
            ...
            {!isRegistering && (
              <div style={{ 
                ...
                <p style={{ color: '#1e293b', fontSize: '15px', fontWeight: 'bold', margin: '0 0 6px 0' }}>
                ...
                <a 
                  href="mailto:asistencia.ecotrack@gmail.com?subject=Restablecer%20Contraseña%20-%20EcoTrack"
                  style={{ 
        ...
        {/* CORRECCIÓN: Beneficios devueltos a su formato transparente y perfectamente alineados en una sola fila */}
        <div style={{ display: 'flex', flexWrap: 'nowrap', justifyContent: 'center', gap: '40px', marginTop: '60px', color: '#1e293b', textAlign: 'center', maxWidth: '1000px', paddingBottom: '40px' }}>
        ...
        {/* Chatbot flotante (Login) */}
        <div style={{ position: 'fixed', bottom: '30px', right: '30px', zIndex: 1000, fontFamily: 'sans-serif' }}>
          {isChatOpen ? (
            <div style={{ width: '340px', height: '480px', backgroundColor: '#fff', borderRadius: '20px', boxShadow: '0 15px 35px rgba(0,0,0,0.2)', display: 'flex', flexDirection: 'column', overflow: 'hidden', border: `2px solid ${colors.primary}`, animation: 'fadeInUp 0.4s ease-out' }}>
            ...
            <button onClick={() => setIsChatOpen(true)} style={{ width: '80px', height: '80px', borderRadius: '50%', backgroundColor: colors.primary, color: '#fff', border: 'none', cursor: 'pointer', fontSize: '36px', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: '0 6px 20px rgba(22, 163, 74, 0.5)', transition: 'transform 0.2s, box-shadow 0.2s' }}>💬</button>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', backgroundColor: '#f3f4f6', display: 'flex', flexDirection: 'column', fontFamily: 'sans-serif', color: '#1f2937' }}>
      {renderEcoAlerta()}

      <header style={{ padding: '15px 50px', backgroundColor: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `5px solid ${colors.primary}`, boxShadow: '0 4px 12px rgba(0,0,0,0.05)', zIndex: 10 }}>
      ...
      <main style={{ padding: '40px', flex: 1, backgroundColor: '#f9fafb' }}>
        
        {/* Banner de bienvenida */}
        <div style={{ ...cardStyle, marginBottom: '35px', borderLeft: `8px solid ${colors.primary}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff', padding: '30px' }}>
          <div>
        ...
        {/* CORRECCIÓN: Contenedor con GRID para asegurar que siempre estén en 1 sola fila horizontal (12345) */}
        <div style={{ ...cardStyle, marginBottom: '35px', backgroundColor: '#fff', border: '1px solid #e5e7eb' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr) auto', gap: '15px', alignItems: 'flex-end', width: '100%' }}>
            {[
              ...
              <div key={idx}>
                ...
                <input 
                  ...
                  style={{...baseInputStyle, width: '100%', borderColor: item.color, borderSize: '3px', fontSize: '16px', padding: '12px'}} 
                  placeholder="0.00"
                />
              </div>
            ))}
            ...
            <button onClick={async () => {
              const lStr = luz.actual.trim();
              const aStr = agua.actual.trim();
              const oStr = residuos.organicos.trim();
              const iStr = residuos.inorganicos.trim();
              const otStr = residuos.otros.trim();

              if (!lStr || !aStr || !oStr || !iStr || !otStr) {
                return mostrarAlerta("Por favor, completa todos los campos del formulario.", 'error');
              }

              if (Number(lStr) <= 0 || Number(aStr) <= 0 || Number(oStr) <= 0 || Number(iStr) <= 0 || Number(otStr) <= 0) {
                return mostrarAlerta("Todos los valores de consumo deben ser mayores a 0.", 'error');
              }
            ...
            }} style={{ padding: '14px 25px', background: colors.primary, color: '#fff', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: 'bold', fontSize: '15px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>GUARDAR DATOS</button>
          </div>
        </div>
        ...
        <div style={{ ...cardStyle, backgroundColor: '#fff', border: `3px solid ${colors.agua}` }}>
                            ...
                            <>
                              <button onClick={async () => {
                                ...
                                if (!elStr || !eaStr || !eoStr || !eiStr || !eotStr) {
                                  return mostrarAlerta("No puedes dejar campos en blanco durante la edición.", 'error');
                                }

                                if (Number(elStr) <= 0 || Number(eaStr) <= 0 || Number(eoStr) <= 0 || Number(eiStr) <= 0 || Number(eotStr) <= 0) {
                                  return mostrarAlerta("Los valores ingresados deben ser mayores a 0.", 'error');
                                }
